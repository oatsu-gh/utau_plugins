...
- 全ノートのフラグにノート長が記入されている場合は、フラグからノート長を削除する。
  -[#0001]` & `Flags=g-2H40` → `Flags=【0001】g-2H40`
- フラグにノート長が記入されていないノートが一つでもある場合は、ノート長を記入する。
  - `Flags=【0001】g-2H40` → `Flags=g-2H40`
"""
import re
...
def copy_tag_to_flags(plugin: up.utauplugin.UtauPlugin):
    """ノート番号をフラグに追記する
    ...
    for note in plugin.notes:
        ...
        note.flags = f'【{note_index}】{str(note.flags)}'


def delete_tag_from_flags(plugin: up.utauplugin.UtauPlugin, pattern: re.Pattern):
    """フラグ内のLengthを消す
...
def main(plugin):
    ...
    if all((note.lyric == 'R' or pattern.search(note.flags)) for note in notes):
        delete_tag_from_flags(plugin, pattern)
    else:
        delete_tag_from_flags(plugin, pattern)
        copy_tag_to_flags(plugin)
